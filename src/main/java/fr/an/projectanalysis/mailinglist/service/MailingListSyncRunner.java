...
import fr.an.projectanalysis.mailinglist.client.MailingListApiClient;
...
public class MailingListSyncRunner {
    ...
    public void syncAll() throws Exception {
        ...
        for (YearMonth month = fromMonth; !month.isAfter(currentMonth); month = month.plusMonths(1)) {
            ...
            List<SourceMailMessageDTO> partitionedItems = apiClient.fetchMbox(yearMonth);
            messageChangeCount += partitionedItems.size();

            repository.savePartitionData(yearMonth, partitionedItems);

            log.info("synced {} : {} messages", yearMonth, partitionedItems.size());
            sleep(syncDelayMs);
        }
