package fr.an.jira.mailinglist.mapper;
 ...
 * {@link MailMessageDTO}, keeping only the fields worth persisting (see that class's javadoc).
 */
public class MimeMessageToMailMessageMapper {

    private static final Pattern MESSAGE_ID_TOKEN = Pattern.compile("<[^<>]+>");
    private static final Pattern HTML_TAG = Pattern.compile("<[^>]+>");

    private MimeMessageToMailMessageMapper() {
    }

    public static MailMessageDTO from(Message src) {
        if (src == null) {
            return null;
        }
        ...
        dest.to = src.getTo() != null
                ? src.getTo().stream().map(MimeMessageToMailMessageMapper::formatAddress).collect(Collectors.toList())
                : List.of();
        dest.cc = src.getCc() != null
                ? src.getCc().stream().map(MimeMessageToMailMessageMapper::formatAddress).collect(Collectors.toList())
                : List.of();
    ...
    private static String formatMailboxes(List<Mailbox> mailboxes) {
        ...
        return mailboxes.stream().map(MimeMessageToMailMessageMapper::formatMailbox)
